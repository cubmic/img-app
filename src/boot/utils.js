      ...
      for (let nr = 0; nr < imageData.data.length; nr += 4) {
        ...
        for (let c = 0; c < gradient.length; c++) {
          if (gradient[c].percent >= lightness) {
            ...
            const pos = $math.invlerp(gradient[min].percent, gradient[c].percent, lightness) // get pos between two gradient colors
            const rgba1 = this.hexToRgba(gradient[min].color)
            const rgba2 = this.hexToRgba(gradient[c].color)
            imageData.data[nr + 0] = $math.lerp(rgba1.r, rgba2.r, pos)
            imageData.data[nr + 1] = $math.lerp(rgba1.g, rgba2.g, pos)
            imageData.data[nr + 2] = $math.lerp(rgba1.b, rgba2.b, pos)
            imageData.data[nr + 3] = $math.lerp(rgba1.a, rgba2.a, pos)
            break
          }
        }
      }
